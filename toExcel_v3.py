...
dfs = pd.read_excel(xlsx_location, 
                    engine= "openpyxl",
                    header= None,
                    usecols= [1],
                    names= ['Patient_Name'],
...
ptdf = dfs.drop(dfs.tail(2).index, inplace=True)
# ptdf = dfs.iloc[::3, :]
pts = ptdf.squeeze()
# mvdf = dfs.iloc[1::3, :].squeeze()
mmrnSS = pts.str.findall(r'\d+').str[0]
nameSS = pts.str.replace(r'\d+', '', regex=True).replace(r'\(\)', '', regex=True)
nameSS = [s.strip() for s in nameSS]
mvstrip = [s.strip() for s in mvdf]
MV_dict = dict((zip(nameSS, mvstrip )))
mmrn_dict = dict((zip(nameSS,mmrnSS)))

mmrn_ls = [val for key, val in mmrn_dict.items() if re.search(Name, key, re.IGNORECASE)]     
MV_ls = [val for key, val in MV_dict.items() if re.search(Name, key, re.IGNORECASE)] 

MMRN_value = list(mmrn_ls)[0]      
MV_value = list(MV_ls)[0]

ws['C5'].value = MMRN_value                                                    #MMRN
